...
return response.Editeurs ;
}
export const fetchEditeurById=async(editeurId)=> {
    const res = await
    fetch(process.env.NEXT_PUBLIC_API_URL+EDITEUR_API+`${editeurId}`, { cache: 'no-store' },
        {
    method: 'GET'
    });
    const response = await res.json();
...
export const deleteEditeur=async(editeurId) =>{
    const res = await
    fetch(process.env.NEXT_PUBLIC_API_URL+EDITEUR_API+`${editeurId}`,
        {
    method: 'DELETE'
    });
    const response = await res.json();
    ...
    export const addEditeur=async(editeur)=> {
        ...
        const res = await
        fetch(process.env.NEXT_PUBLIC_API_URL+EDITEUR_API+`${editeur._id}`, {
        method: 'PUT',
        headers: {
        'Content-Type': 'application/json',
        },
