...
export async function PUT (req, { params }) {try {
    await connectDB();
    ...
    const livre = await Livre.findById(params.id);
    if (livre) {
    await Livre.findByIdAndDelete(livre._id);return NextResponse.json({ message: `Book ${params.id} has been
        deleted` });
        }
        return NextResponse.json({ message: `Book ${params.id} not found` }, {
        status: HttpStatusCode.NotFound });
        } catch (error) {
        return NextResponse.json({ message: error }, { status:
        HttpStatusCode.BadRequest });
